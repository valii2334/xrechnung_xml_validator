  ...
  context 'validate!' do
    ...
    context 'file is not found' do
      ...
      it 'raises XRechnungValidator::Errors::FileNotFound' do
        expect { subject }.to raise_error(XrechnungXmlValidator::FileNotFound, "#{absolute_path} not found")
      end
    end

    context 'file is found' do
      let(:error_message) do
        "#{absolute_path} must have format #{XrechnungXmlValidator::FileValidator::VALID_FORMATS.join(', ')}"
      end
